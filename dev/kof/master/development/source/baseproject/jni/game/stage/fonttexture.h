...
struct FontTexture::SurfaceCreateParams
{
	float scale; // 0x0
	long letterSpacing; // 0x4
	SurfaceCreateParams();
};
struct FontTexture::RenderSurface
{
	void * pBuffer; // 0x0
	long width; // 0x8
	long height; // 0xC
	long textWidthSize; // 0x10
	long textHeightSize; // 0x14
	long pixeSizeByte; // 0x18
	long bufWidthByte; // 0x1C
	long skipSymbolCount; // 0x20
	RenderSurface();
	void Clear();
};