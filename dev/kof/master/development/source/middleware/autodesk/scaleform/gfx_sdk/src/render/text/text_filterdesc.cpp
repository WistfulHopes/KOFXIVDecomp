...
void Scaleform::Render::Text::TextFilter::SetDefaultShadow(); // 0x1403800E0
void Scaleform::Render::Text::TextFilter::LoadFilterDesc(const Scaleform::Render::Filter * filter); // 0x14037FFD0void __fastcall Scaleform::Render::Text::TextFilter::LoadFilterDesc(
        ...
        const Scaleform::Render::Filter *filter)
{
  ...
  if ( Type )
  {
    if ( (unsigned int)(Type - 1) <= 1 && (!this->ShadowParams.Colors[0].Raw || this->ShadowDistance == 0.0) )
    {
      this->ShadowFlags = 0;
      v4 = 0;
      if ( ((__int64)filter[1].__vftable & 0x10) != 0 )
        v4 = 32;
      this->ShadowFlags = v4;
      if ( ((__int64)filter[1].__vftable & 0x40) != 0 )
        this->ShadowFlags |= 0x40u;
      if ( HIDWORD(filter[1].__vftable) )
      ...
      this->ShadowParams.Strength = *(float *)&filter[2].__vftable;
      *(_QWORD *)&this->ShadowParams.Colors[0].Channels.Blue = *(Scaleform::Render::Filter_vtbl **)((char *)&filter[2].__vftable
                                                                                                  + 4);
      this->ShadowAlpha = this->ShadowParams.Colors[0].Channels.Alpha;
      this->ShadowAngle = *((float *)&filter[3].__vftable + 1);
      v5 = *(float *)&filter[3].__vftable;
      ShadowAngle = this->ShadowAngle;
      this->ShadowDistance = v5;
      v7 = cosf(ShadowAngle) * v5;
      v8 = sinf(this->ShadowAngle);
      this->ShadowParams.Offset.x = v7;
      this->ShadowParams.Offset.y = v8 * v5;
    }
  }
  else
  {
    ...
    this->BlurStrength = *(float *)&filter[2].__vftable;
  }
}
